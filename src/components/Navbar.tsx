    ...
    return (
        <nav className="h-20 w-full fixed top-0 left-0 border-b-[1px] border-zinc-200 flex items-center justify-between px-9 bg-white z-50">
            <div className='flex items-center gap-6'>
                <button onClick={() => setShowSidebar(!showSidebar)} className='w-20 -ml-4 h-12 border rounded-lg text-zinc-700 hover:bg-gray-200 transition-all'>{showSidebar ? <DoubleArrowIcon className='lg:rotate-180 rotate-90' /> : <MenuIcon />}</button>
